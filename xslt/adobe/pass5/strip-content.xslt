...
<xsl:stylesheet version="2.0"
    ...
    <!-- Content stripping processing (Pass 5, mode="pass5") - Remove elements marked with data-strip="true" until data-strip="false" -->

    <!-- Identity transform for pass5 (copy everything by default) -->
    ...
    <xsl:template match="*[p or h1 or h2 or h3 or h4 or h5 or h6 or img or table]" mode="pass5" priority="5">
        <xsl:copy>
        ...
        <xsl:if test="$position le count($nodes)">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$current[self::p or self::h1 or self::h2 or self::h3 or self::h4 or self::h5 or self::h6 or self::img or self::table][@data-strip='false']">
                    <xsl:apply-templates select="$current" mode="pass5-strip-attr"/>
                    <xsl:call-template name="process-with-stripping">
                        <xsl:with-param name="nodes" select="$nodes"/>
                ...
                <xsl:otherwise>
                    <xsl:apply-templates select="$current" mode="pass5"/>
                    <xsl:call-template name="process-with-stripping">
                        <xsl:with-param name="nodes" select="$nodes"/>
                        <xsl:with-param name="stripping" select="false()"/>
                        <xsl:with-param name="position" select="$position + 1"/>
    ...
    <xsl:template match="@data-strip" mode="pass5" priority="10"/>

    <!-- Mode pass5-strip-attr: Like pass5 but explicitly removes data-strip attribute -->
    <xsl:template match="node()" mode="pass5-strip-attr">
        <xsl:copy>
            <xsl:apply-templates select="@*[not(local-name() = 'data-strip')]" mode="pass5"/>
            <xsl:apply-templates select="node()" mode="pass5"/>
        </xsl:copy>
    </xsl:template>

    <xsl:template match="@*" mode="pass5-strip-attr">
        <xsl:copy/>
    </xsl:template>
